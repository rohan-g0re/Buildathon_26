# Layer 0 — Example Data Format

This document shows what the raw data from Layer 0 should look like. All data is in **Markdown format** — structured with tables, headers, and prose. Use this as the template when creating synthetic data.

The example company is **NovaTech Inc. (NVTK)**, a mid-cap enterprise SaaS company.
...
This is what `fetch_financial_data("NVTK")` should return — a markdown string. Layer 1's financial inference agent consumes this to produce F1.

---

### BEGIN EXAMPLE — Financial Data for NVTK
...
This is what `fetch_news_data("NVTK")` should return — a markdown string. Layer 1's trend inference agent consumes this to produce F2.

---

### BEGIN EXAMPLE — News & Sentiment Data for NVTK
...
### END EXAMPLE — News & Sentiment Data
...
When building synthetic data for a different company:

1. **Pick a narrative.** Every company has a story. NovaTech's story is: "decelerating growth company making an AI pivot while facing competitive pressure and governance concerns." Your synthetic company needs a story with tension.

2. **Make the tables tell the story.** Revenue, margins, and ratios should trend in a way that matches the narrative. Don't randomize — make it internally consistent.

3. **Include conflicting signals.** Good consulting data has bulls vs bears. Some metrics look great, others are concerning. That's what forces Layer 1 and 2 to actually reason.

4. **Include events and people.** Management changes, acquisitions, lost customers, product launches. These make the analysis non-generic.

5. **Include competitors.** At least 1-2 with recent moves. Consulting always needs competitive context.

6. **Keep the structure identical.** Same section headers, same table formats. The agents will expect this layout. Only change the content.
