...
6. **Frontend components are atomic.** `PipelineView` composes `LayerCard`s. `LayerDetail` composes `AgentNode`s and `DocumentViewer`s. Each component is a single `.tsx` file.
